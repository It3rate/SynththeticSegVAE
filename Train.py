...
class TrainVAE:
    def __init__(self, csv_path:str, batch_size:int, learning_rate=1e-3, model_path:str = ""):
        ...
        self.start_epoch = 0
        self.model = VAEmodel(latent_dims=VAEmodel.latent_dimensions, hidden_dims=[32, 64, 64], image_shape=[3,32,32])
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            for batch in self.train_dataloader:
                ...
                recons, input, mu, log_var, _ = self.model.forward(img, gen)
                self.loss = self.model.loss_function(recons, input, mu, log_var)
                self.loss.backward()
                self.optimizer.step()
...
class TrainDecoder:
    def __init__(self, csv_path:str, batch_size:int, output_dir:str, learning_rate=1e-3, model_path:str = ""):
        #os.chdir(os.path.dirname(os.path.abspath(__file__)))
        self.epochs = 1000
